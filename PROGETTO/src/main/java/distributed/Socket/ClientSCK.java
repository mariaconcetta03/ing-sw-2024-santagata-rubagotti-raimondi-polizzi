...
public class ClientSCK {//here we can have the view attributes that would be modified after we read the buffer
    ...
    public ClientSCK(String address, int port) throws IOException, ClassNotFoundException { //we call this constructor after we ask the IP address and the port of the server
        ...
        while (true) {
            try {
                SCKMessage obj = (SCKMessage) this.inputStream.readObject(); //così non abbiamo più bisogno della funzione receiveMessage
                update(obj);
            } catch (Exception e) {
                break; //se per esempio il flusso viene interrotto
            }
    ...
    //qualunque sia update da fare dovrebbe venire chiamata per dire al server tutto okay
    public void sendMessage(SCKMessage mess) throws IOException { //we write the socket so that if the Thread reads this stream, it can receive the message
        try {
    ...
    public void update(SCKMessage mess) {
